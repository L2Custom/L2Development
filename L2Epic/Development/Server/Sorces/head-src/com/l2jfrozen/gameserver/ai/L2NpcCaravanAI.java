...
import com.l2jfrozen.gameserver.datatables.csv.NpcCaravanRoutesTable;
...
public class L2NpcCaravanAI extends L2CharacterAI implements Runnable
{
	 ...
	 */
	private final List<L2NpcCaravanNode> route = NpcCaravanRoutesTable.getInstance().getRouteForNpc(getActor().getNpcId());
	 ...
	 */
	public L2NpcCaravanAI(final L2Character.AIAccessor accessor)
	{
		super(accessor);
	...
	{
		LOGGER.warn("NpcCaravan ID: " + getActor().getNpcId() + ": Blocked at rote position [" + currentPos + "], coords: " + blocked_at_pos.x + ", " + blocked_at_pos.y + ", " + blocked_at_pos.z + ". Teleporting to next point");
	...
	private void checkArrived()
	{
		...
		if (getActor().getX() == destinationX && getActor().getY() == destinationY && getActor().getZ() == destinationZ)
		{
			...
			if (chat != null && !chat.equals("NULL"))
			{
				...
				{
					LOGGER.error("L2NpcCaravanInstance: Error, " + e);
				}
			}
			chat = null;
