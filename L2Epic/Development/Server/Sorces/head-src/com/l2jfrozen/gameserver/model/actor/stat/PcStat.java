...
public class PcStat extends PlayableStat
{
	...
	public boolean addExpAndSp(long addToExp, int addToSp)
	{
		...
		SystemMessage sm = new SystemMessage(SystemMessageId.YOU_HAVE_EARNED_S1_EXPERIENCE_AND_S2_SP);
	...
	public boolean removeExpAndSp(final long addToExp, final int addToSp)
	{
		...
		// Send a Server->Client System Message to the L2PcInstance
		SystemMessage sm = new SystemMessage(SystemMessageId.YOU_EXPERIENCE_HAS_DECREASED_BY_S1);
		sm.addNumber((int) addToExp);
		getActiveChar().sendPacket(sm);
		sm = null;
	...
	public final boolean addLevel(final byte value)
	{
		...
		if (levelIncreased)
		{
			...
			getActiveChar().sendPacket(new SystemMessage(SystemMessageId.YOUR_LEVEL_HAS_INCREASED));
